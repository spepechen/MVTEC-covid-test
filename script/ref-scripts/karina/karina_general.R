...
setwd("D:/karina/docencia/3mastersPropis/masterVisualization/StatsProg/practica")

#dd <- read.table("owid-covid-data.csv",header=T);

dd<-read.csv("D:/karina/docencia/3mastersPropis/masterVisualization/StatsProg/practica/owid-covid-data.csv")
...
#extra data with country info
ddExtra<-read_excel("InfoPaisosExtra.xlsx")
...
barplot(table(ddExtra$Government_Type), las=3, cex.names=0.7, main=paste("Barplot of", names(ddExtra$Government_Type)))

GovType<-factor(ddExtra$Government_Type)
levels(GovType)

GovTypeRephrased<-GovType

newvalues<-c("AbsMonar","Communist","ConstMonar","Dictatorship","Transition", "IslParRep",
             "IslPreRep","IslSPreRep", "ParRep", "PreLimDemo","PreRep", "SPreRep")
GovTypeRephrased <- newvalues[ match(GovTypeRephrased,levels(GovType)) 
]

barplot(table(GovTypeRephrased), main=paste("Barplot of", names(ddExtra$Government_Type)))
barplot(table(GovTypeRephrased), cex.names=0.7,main=paste("Barplot of", names(ddExtra$Government_Type)))
barplot(table(GovTypeRephrased), las=3, cex.names=0.7, main=paste("Barplot of", names(ddExtra$Government_Type)))

#interpolate missing data for numerical variables 
#install.packages("imputeTS")
library(imputeTS)
X<-na_interpolation(X)
#X is the series to be interpolated. Do not mmix data from different countries here

CountryPopul<-aggregate(InfoPaisosExtra$Population_total, by=list(GovTypeRephrased), FUN=mean)
...
for(k in 1:7){InfoPaisosExtra[,k]<-factor(InfoPaisosExtra[,k])}
actives<-c(1:10)
...
#filtro<-which(dd[,3]%in% c("lloguer","pares"))
#dissimMatrix <- daisy(dd[filtro,actives], metric = "gower", stand=TRUE)

filtro<-InfoPaisosExtra[1:10,]

dissimMatrix <- daisy(InfoPaisosExtra[,actives], metric = "gower", stand=TRUE)
