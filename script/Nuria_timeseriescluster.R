...
selecteddf <- df %>% select(iso_code, date , new_cases_smoothed_per_million, stringency_index)%>%
...
ggplot(selecteddf, aes(date, new_cases_smoothed_per_million, group=iso_code, color=iso_code))+ geom_line()+xlab("")+ theme(legend.position = 'none')


#Prepare data for cluster analysis

###old version, useful only for 2 variables:
###   selecteddf_spread <- selecteddf %>%
###   spread(iso_code, total_cases_per_million) %>%  
###   deaths<- t(selecteddf_spread [-1])


selecteddfpivot <- selecteddf %>%  
  pivot_wider(id_cols=c('iso_code','new_cases_smoothed_per_million'),
              names_from='date',
              names_sep = ".",
              values_from=c('new_cases_smoothed_per_million', "stringency_index"),
              values_fill=0) %>%
  remove_rownames() %>%
  column_to_rownames("iso_code")
...
ggplot(joined_clusters, aes(date, new_cases_smoothed_per_million, group=iso_code, color=cluster))+
  geom_line()+xlab("")+ 
