...
# Second set for usa_original which includes longer time frame
set.seed(567)
...
# Build the model
newcases_model <- lm(usd2twd_avg ~ new_cases, data = train.data ) # fit the model
newcases_model_original <- lm(usd2twd_avg ~ new_cases, data = usa_original ) # fit the model

# Make predictions
newcases_predictions <- newcases_model %>% predict(test.data)
# Model performance
# we want low RMSE and high R2
# 0.1 RMSE seems low enough but put in the context of the currency conversion usd2twd... then it might not be?
# R2 = 0.78 
data.frame(
...
plot(usa$new_cases, usa$usd2twd_avg, main="From Oct")
abline(newcases_model, col="blue")

summary(newcases_model_original)
plot(usa_original$new_cases, usa_original$usd2twd_avg,main="From March")
...
usa_selected <- usa %>% select(usd2twd_avg, yest_new_cases_smoothed, yest_new_deaths_smoothed)
plot(usa_selected)
...
usa_selected <- usa %>% select(usd2twd_avg, yest_new_cases, yest_new_deaths)
plot(usa_selected)
